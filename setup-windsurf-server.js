...
const windsurfConfigPath = isWindows
  ? join(process.env.APPDATA, "Windsurf", "mcp.json")
  : join(homedir(), ".windsurf", "mcp.json");

// Setup logging
const LOG_FILE = join(__dirname, "windsurf-setup.log");
...
(async function main() {
  try {
    ...
    const configDir = dirname(windsurfConfigPath);
    ...
    if (existsSync(windsurfConfigPath)) {
      logToFile(`Found existing Windsurf MCP config at: ${windsurfConfigPath}`);
      try {
        const configData = readFileSync(windsurfConfigPath, "utf8");
      ...
      logToFile(
        `No existing Windsurf MCP config found. Creating new config at: ${windsurfConfigPath}`
      );
    }
    ...
    writeFileSync(windsurfConfigPath, JSON.stringify(config, null, 2), "utf8");

    logToFile("Successfully updated Windsurf MCP configuration!");
    logToFile(`Configuration location: ${windsurfConfigPath}`);
    logToFile(
      "\nTo use the MCP tools:\n1. Restart Windsurf if it's currently running\n2. The tools will be available in Windsurf's MCP server list"
    );
  } catch (error) {
    logToFile(`Error updating Windsurf configuration: ${error}`, true);
    process.exit(1);
  }
})();
